...
**第三层是检索层。** 我们用了 **Milvus 向量数据库** 做课程 RAG 和对话记忆。检索方式不是单纯的向量相似度，而是 **BM25 稀疏匹配 + Dense 稠密检索 + RRF 加权融合**。原因很简单——课程代码 "COMP7940" 这种精确标识符在语义空间里是稀疏的，COMP7940 和 COMP7930 的 embedding 距离可能比 COMP7940 和 "biology" 还近。纯稠密检索经常混淆，加上 BM25 做关键词精确匹配就能互补。阈值设在 0.5，低于这个值如实告知用户"没有相关信息"，不编造。
