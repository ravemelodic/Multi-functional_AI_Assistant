...
**另外在检索层里，PDF 分析我们做了个"上传即问答"的设计。** 用户上传 PDF 时如果带 caption，比如"只分析第三章"，系统会通过 Celery 提取全文，然后分块 embed 入库。不走传统的"先摘要再给人看"的流程，而是让 `retrieve_rag` 直接从刚入库的块里检索跟"第三章"相关的内容，只取 Top-5 块喂给 LLM。同一次对话里就能得到针对性回答，不用等第一轮出摘要再第二轮追问。同时 `skip_memory=True` 跳过对话记忆，避免历史记录干扰。
