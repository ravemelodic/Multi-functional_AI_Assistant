...
所以我们做了 **三级异步卸载**——文本走 Redis 队列，Bot 进程内部 20 并发消费；PDF 走 Celery OCR 池 80 并发；视频走 Celery 视频池 20 并发。同一条消息只会走其中一条路径，但最终在 `retrieve_rag → build_prompt → call_llm` 完全复用下游逻辑。**分叉后还能汇合，这是纯 LangChain 线性管道做不到的。**

**第三层是检索层。** 我们基于 **LangChain + Milvus** 做课程 RAG 和对话记忆——通过 LangChain 的 `VectorStoreRetriever` 和 `similarity_search_with_relevance_scores` 操作 Milvus 向量库。检索方式不是单纯的向量相似度，而是 **BM25 稀疏匹配 + Dense 稠密检索 + RRF 加权融合**。原因很简单——课程代码 "COMP7940" 这种精确标识符在语义空间里是稀疏的，COMP7940 和 COMP7930 的 embedding 距离可能比 COMP7940 和 "biology" 还近。纯稠密检索经常混淆，加上 BM25 做关键词精确匹配就能互补。阈值设在 0.5，低于这个值如实告知用户"没有相关信息"，不编造。
