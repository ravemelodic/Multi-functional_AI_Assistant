...
> 项目内置多层安全防护机制，涵盖认证鉴权、通信加密、存储隔离、容器加固、审计追溯和日志脱敏。

### 7.1 管理员认证（Token-Based RBAC）

管理后台采用 **Token 认证 + 角色权限控制（RBAC）**：
...
- Token 通过 `ADMIN_TOKENS_JSON` 环境变量注入，不写死代码
- 每人独立 Token，单独踢人无需影响他人
- 支持 `X-API-Key` Header、URL 参数、Cookie 三种传递方式
- 新增管理员：生成随机 Token → 加入 JSON 字符串 → 重启容器

### 7.2 通信安全（HTTPS）

- API 容器使用 **自签 SSL 证书**（Dockerfile 构建时生成，有效期 10 年）
- uvicorn 启动时加载 `--ssl-certfile` / `--ssl-keyfile`
- 管理后台所有请求强制 HTTPS
...
| 组件 | 安全措施 |
|------|----------|
| **Redis** | 密码认证（通过 `.env` 文件配置 `REDIS_PASSWORD`） |
| **Milvus** | 无宿主机端口暴露，仅 Docker 内网可达 |
| **MinIO** | 无宿主机端口暴露，仅 Docker 内网可达 |
| **配置** | `config.ini`、`.env` 均在 `.gitignore` 中 |

所有内部服务（Redis、Milvus、etcd、MinIO） **不暴露宿主机端口**，通过 Docker 内部网络通信。唯一对外入口是 `api:8000`（HTTPS + Token 认证）。
...
read_only: true          # 根文件系统只读（攻击者无法篡改代码）
tmpfs: /tmp              # 临时目录独立 tmpfs 挂载
```
...
| 事件 | 记录内容 |
|------|----------|
| 登录成功/失败 | Token 前 12 位、角色、IP、时间 |
| 上传文件/JSON 导入 | 操作人、文件名/记录数、成功/失败、IP |
...
| 级别 | 记录方式 | 场景 |
|------|----------|------|
| `INFO`（默认） | 只记前 50 字符 + `...` | 日常运维 |
| `DEBUG` | 完整原文 | 排查 bug 时临时开启 |

同时记录：用户 ID、意图分类（intent）、响应耗时、错误信息、熔断/限流事件。

### 7.7 文件上传安全

- 最大 **10 MB**（超限返回 HTTP 413）
- 仅允许 `.csv` / `.json` 扩展名
- 上传需 `upload` 角色 Token 认证

### 7.8 密钥管理

- 密钥优先通过环境变量注入（docker-compose environment / `.env` 文件）
- `ADMIN_TOKENS_JSON` 直接以环境变量形式传入，不走 config.ini
- 敏感文件（`.env`、`config.ini`）已在 `.gitignore` 中
- Token 生成命令：`python3 -c "import secrets; print(secrets.token_hex(32))"`

---

## 8. 最新变更（v2.0 → v2.1）

### 8.1 general_chat 统一走 RAG 检索
