...
│  │    │                                                         │  │
│  │    ├── analyze_document → Celery OCR + 入库 → retrieve_memory
...
| `analyze_document` | `analyze_document → END` | `analyze_document → retrieve_memory → LLM`  + 异步入库 | **汇入对话 pipeline + caption 指令** |
...
**动机**：用户上传 PDF 时无法指定解析范围（如"只分析第三章"），必须等全文档摘要后在第二轮对话提问，但此时 PDF 原文已不在上下文中。
...
| 路由 | `analyze_document → END` | `analyze_document → retrieve_memory → ... → call_llm` |
| 返回方式 | 直接返回 Celery 摘要作为 `final_response` | 注入 `rag_context`（全文+摘要），走对话 pipeline |
| 回答能力 | 固定全文档摘要 | 可回答"第三章讲什么"、"截止日期有哪些"等具体问题 |
...
用户上传 PDF + caption "只分析第三章"
  ...
  │   ├── Celery OCR 提取全文
  │   ├── 异步 ingest_text() 入库（fire-and-forget）
  │   └── 返回 {"rag_context": 全文+摘要, "rag_empty": False}
  │
...
“纯 LangChain 的 LCEL 适合做无状态的线性管道（比如单纯的 RAG 问答）。但我们的项目涉及多轮视频交互（中断恢复）和记忆存储与读取的闭环，这需要框架具备循环图执行能力和内置的检查点机制。如果用纯 LangChain，我需要在外层手动维护一个庞大的 Redis 状态机，并编写大量胶水代码来处理分支汇合，这既容易出错，也丧失了 LangChain 官方对代理工作流的最佳实践支持。LangGraph 负责’指挥调度’，LangChain 负责’工具实现’，两者组合才是最优解。”
...
**3. 长文档的 Token 瓶颈**
OCR 提取的 PDF 动辄几万字，直接塞进 prompt 会打爆 Token 限制。我们做了**分级摘要**策略：先对每 3000 字段落独立摘要，再二次合成，确保关键信息不丢失。RAG 分块则是 500 字 + 50 重叠，在检索粒度和 embedding 质量之间取平衡。
...
**4. 文档的”上传即问答”**
竞品的文档分析通常是”上传 → 等待 → 看摘要”的单向流程。我们打通了 **OCR 提取 → 注入 RAG → 对话 pipeline 继续执行** 这条链路，用户上传 PDF 时附带 caption 即可指定解析范围，AI 在提取全文后直接回答具体问题，同时内容自动入库方便后续检索。整个过程在一次图执行中完成。
