...
这是一个面向香港浸会大学学生的 **Telegram 智能助手中台**——学生在 Telegram 上跟它对话，可以查课程信息、问作业截止日期、上传 PDF 让 AI 总结、甚至把图片转成视频。

---

**核心架构**

最上层是 **Telegram Bot**，接收用户消息。消息进来后进入一个 **LangGraph 状态机**——它先做意图分类，然后路由到不同处理节点，最终汇入同一条 LLM 调用流水线。

```
消息 → 意图分类 → 三选一
  ├── 文本对话 → Redis 队列 → 后台 20 并发消费 → RAG + LLM → 回复
  ├── PDF 文档  → Celery OCR 池（80 并发）→ 提取全文 → 入库 → RAG → LLM → 回复
  └── 视频生成  → Celery 视频池（20 并发）→ 外部 API → 返回视频
```

三个路径最终都汇聚到同一套 **检索 + 生成管线**：先查 Milvus 向量库（BM25 + 稠密混合检索），再查对话记忆，最后喂给 ChatGPT 生成回答。

---

**几个关键设计决策**

**第一，为什么用 LangGraph 而不是纯 LangChain？**

因为业务涉及三类不同粒度的任务——文本对话 ~2s、PDF 分析 ~60s、视频生成 ~300s。纯 LangChain 的线性管道没法让这三条路径分叉后汇合。LangGraph 的图结构允许在 `classify_intent` 处分叉，在 `retrieve_rag → build_prompt → call_llm` 处汇合，共享 80% 的下游代码。

**第二，为什么检索用了 BM25 + Dense 双引擎？**

课程代码 "COMP7940" 这种精确标识符在语义空间里非常稀疏——COMP7940 和 COMP7930 的 embedding 距离可能比 COMP7940 和 "biology" 还近。纯稠密检索经常混淆。加了 BM25 做关键词精确匹配，RRF 融合排序，0.5 阈值兜底。

**第三，PDF"上传即问答"怎么做到的？**

用户上传 PDF 时如果带 caption（如"只分析第三章"），Celery 提取全文后不是直接返回摘要，而是分块入库，然后走 `retrieve_rag` 语义检索，只取跟问题相关的 Top-5 块喂给 LLM。同时 `skip_memory=True` 跳过对话记忆避免干扰。整个流程在同一次图执行里完成。

**第四，削峰填谷怎么做的？**

原来 `Semaphore(50)` 直接拒绝第 51 个请求。后来改成 Redis 消息队列——文本请求全部入队，后台 20 并发稳定消费。瞬时洪峰时用户不会被拒，只是多等几秒。Redis 不可用时自动回退同步。

---

**高可用保障**

Azure OpenAI 客户端集成熔断器（连续 5 次失败切断 30 秒）与指数退避重试；内置每用户速率限制（30 次/分钟）与全局并发节流；Redis 消息队列削峰填谷（文本请求全部入队，0 拒绝）；Celery + Redis 异步任务队列支持 3 次自动重试与指数退避；Milvus 断连自动重建 TTL 缓存连接；Docker healthcheck + restart 保障服务健康。

---

**总结**

这个项目本质上是一个**用状态机驱动的多模态 AI 中台**。最特别的地方在于：三种不同粒度的任务（文本、文档、视频）通过同一个状态机调度，各自卸载到不同队列处理，下游检索和生成逻辑完全复用。以上就是我的介绍。
